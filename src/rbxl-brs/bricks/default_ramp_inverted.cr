module RBXLBRS
  class DefaultRampInverted < Part
    def write_xml(xml : XML::Builder)
      ...
      xml.element "Item", {"class" => "WedgePart"} do
        xml.element "Properties" do
          ...
          (cframe(apply_angle: false) * CFrame.angles(0, (rotation - 1) * Math::PI / 2, 0) * CFrame.new(0_f64, 0_f64, -0.5) * CFrame.angles(0.0, 0.0, Math::PI)).write_xml(xml) # 3 - rotation
          write_color(xml)
